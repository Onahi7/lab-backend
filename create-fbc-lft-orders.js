/**
 * Create FBC and LFT orders with proper test arrangement and age-specific ranges
 */
...
const patients = [
  {
    ...
    email: 'dickson.hardy@example.com',
  },
  {
    firstName: 'Jallor',
    lastName: 'Dabiu',
    gender: 'M',
    dateOfBirth: new Date(Date.now() - (2 * 24 * 60 * 60 * 1000)),
    age: 0,
    phoneNumber: '+1234567891',
    email: 'jallor.dabiu@example.com',
  },
];
...
function generateValue(refRange) {
...
async function main() {
  try {
    ...
    for (const patient of patients) {
      ...
      console.log(`✅ ${patient.firstName} ${patient.lastName} (${patient.age} ${patient.age === 0 ? 'days' : 'years'}, ${patient.gender})`);
    }
    console.log('');
    ...
    for (let i = 0; i < patientIds.length; i++) {
      ...
      for (const test of sortedFbcTests) {
        ...
        const value = generateValue(refRange);
    ...
    console.log('🖨️  Search for these orders in the application:');
    orders.forEach((o, i) => {
      console.log(`   ${i + 1}. ${o.orderNumber} - ${patients[i].firstName} ${patients[i].lastName} (${patients[i].age} ${patients[i].age === 0 ? 'days' : 'years'})`);
    });
